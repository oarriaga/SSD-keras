...
def record_detections(model, dataset_name, data_prefix, image_prefix,
                      iou_threshold=.5, iou_nms_threshold=.45):
    ...
    for image_name in tqdm(image_names):
        ...
        # print(image_counter / num_images)
        ...
        predicted_classes.append(np.max(predicted_data[:, 4:], axis=1))
        predicted_scores.append(np.argmax(predicted_data[:, 4:], axis=1))
        ...
        ground_truth_classes.append(np.argmax(ground_truth_sample[:, 4:],
                                    axis=1))
        difficult_objects.append(difficult_data_flags[image_name])
