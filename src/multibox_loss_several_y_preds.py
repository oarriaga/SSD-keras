...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred, y_pred_2):
        batch_size = K.shape(y_true)[0]
        num_prior_boxes = K.cast(K.shape(y_true)[1], 'float')
        # is this correct?
        print(type(y_pred))

        y_pred_localization = y_pred
        y_pred_classification = y_pred_2
        # loss for all priors boxes
        localization_loss = self._l1_smooth_loss(y_true[:, :, :4], y_pred_localization)
        classification_loss = self._softmax_loss(y_true[:, :, 4:], y_pred_classification)

        int_positive_mask = 1 - y_true[:, :, 4 + self.background_id + 1]
