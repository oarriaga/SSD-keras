...
ObjectDetectionEvaluation is a class which manages ground truth
information of a object detection dataset, and computes frequently
used detection metrics such as Precision, Recall, CorLoc of the
provided detection results. It supports the following operations:
...
class ObjectDetectionEvaluation(object):
    ...
    def add_single_ground_truth_image_info(self,
        ...
        object box coordinates[y_min, x_min, y_max, x_max]
        groundtruth_class_labels: A 1-d numpy array of length M
        representing class labels
        groundtruth_is_difficult_list: A length M numpy boolean
        array denoting whether a ground truth box is a difficult
        instance or not. To support the case that no boxes are difficult,
        it is by default set as None.
        """
        if image_key in self.groundtruth_boxes:
            # DB stands for database
            message = 'image %s has already been added to the ground truth DB.'
            logging.warn(message, image_key)
        return
    ...
    def add_single_detected_image_info(self, image_key, detected_boxes,
        ...
        labels
        Raises:
        ValueError: if detected_boxes, detected_scores and
        detected_class_labels do not have the same length.
        """
        ...
        if (len(detected_boxes) != len(detected_scores) or
           len(detected_boxes) != len(detected_class_labels)):
            raise ValueError('detected_boxes, detected_scores and '
                             'detected_class_labels should all have same'
                             'lengths. Got [%d, %d, %d]' % len(detected_boxes),
                             len(detected_scores), len(detected_class_labels))

        if image_key in self.detection_keys:
            logging.warn(
                'image %s has already been added to the detection result DB',
                image_key)
        return
    ...
    def _update_ground_truth_statistics(self, groundtruth_class_labels,
        ...
        groundtruth_is_difficult_list: A boolean numpy array of length
        M denoting whether a ground truth box is a difficult instance or not
        """
    ...
    def evaluate(self):
        ...
        precisions_per_class: List of precisions, each precision is
        a float numpy array
