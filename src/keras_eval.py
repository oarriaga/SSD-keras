...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    for i in range(num_images):
        ...
        # detections_size = len(detections)
        detection_size = 25  # num classes
        for j in range(1, detection_size):
            ...
            class_mask = best_classes == j
            if np.all(~class_mask):
                continue
            class_scores = class_scores[class_mask]
            boxes = boxes[class_mask]
            cls_dets = np.concatenate((boxes, class_scores), axis=1)
